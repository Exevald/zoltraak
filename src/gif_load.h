	...
	uint8_t* bptr, uint8_t* blen)
{
	...
	for (curr = ++ctbl; curr; code[--curr] = 0)
		; /** actual color codes **/
...
#pragma pack(pop)
	...
	for (whdr.bptr += GIF_BLEN, whdr.ifrm = -1; blen /** load all frames **/
		 ...
		 size = (desc != GIF_EOFM) ? ((desc != GIF_FHDM) || (skip > whdr.ifrm)) ? _GIF_SkipChunk(&buff, size) - 1 : size - 1 : -1)
		...
		{ /** found a frame **/
			...
			*(void**)&whdr.cpal = (void*)(ghdr + 1); /** interlaced? -^ **/
			...
			if ((skip <= ++whdr.ifrm) && ((whdr.clrs <= 0) || (_GIF_LoadFrame(&buff, &size, whdr.bptr, whdr.bptr + blen) < 0)))
			...
			else if (skip <= whdr.ifrm)
			{
				...
				whdr.time = (egch && (egch->flgs & 0x02)) ? -whdr.time - 1
														  : whdr.time;
				whdr.mode = (egch && !(egch->flgs & 0x10)) ? (egch->flgs & 0x0C) >> 2 : GIF_NONE;
				egch = 0;
				wtmp = whdr;
